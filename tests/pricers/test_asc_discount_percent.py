...
from myelin.pricers.asc.client import AscClient, AscLineOutput
...
class TestAscDiscountPercent(unittest.TestCase):
    def test_discount_percent_calculation(self):
        ...
        client.data_loader.get_data = MagicMock(
            return_value={"wage_indices": {"10000": 1.0}, "rates": {}, "code_pairs": {}}
        )
        ...
        claim.lines = [
            ...
            LineItem(
                line_number=2, hcpcs="B", units=2
            ),  # Second procedure, 2 units (higher rate, so it gets sorted first)
